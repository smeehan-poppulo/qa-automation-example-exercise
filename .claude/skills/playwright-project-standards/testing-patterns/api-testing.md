...
**Use when**: Multiple tests share the same endpoints — typed helpers centralise the raw `request.*` call and its return shape, so callers never need to cast or guess the response structure.
**Avoid when**: An endpoint is called in only one test; inline `request.post` with a local type assertion is simpler.
...
export interface Item {
  ...
  title: string;
  price: number;
  category: 'tools' | 'hardware' | 'consumables';
...
```typescript
// helpers/items-api.ts
import { APIRequestContext, expect } from '@playwright/test';
import type { CreateItemRequest, Item, PaginatedResponse } from '../types/api';

export async function createItem(request: APIRequestContext, data: CreateItemRequest): Promise<Item> {
...
export async function getItems(
  ...
  const resp = await request.get('/api/items', { params });
  expect(resp.ok()).toBeTruthy();
  return resp.json() as Promise<PaginatedResponse<Item>>;
}

export async function deleteItem(request: APIRequestContext, id: number): Promise<void> {
  const resp = await request.delete(`/api/items/${id}`);
...
import { test, expect } from '@playwright/test';
import { createItem, getItems, deleteItem } from '../../helpers/items-api';

test('created item appears in list', async ({ request }) => {
  const item = await createItem(request, { title: 'Hammer', price: 19.99, category: 'tools' });
  // item is fully typed — IDE autocomplete works, typos are compile errors
  expect(item.id).toEqual(expect.any(Number));

  const { items } = await getItems(request, { page: 1, limit: 50 });
...
Pair with Zod schema validation (see below) when you also need runtime guarantees — typed helpers give compile-time safety, Zod catches backend drift at runtime.
